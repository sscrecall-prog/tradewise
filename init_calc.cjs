...
const calcServiceCode = `import { JournalEntry, SetupType, EmotionType, DisciplineMetrics, AnalyticsSummary } from '../types';
...
// src/services/TradingCalculationService.test.ts
const testServiceCode = `import { describe, it, expect } from 'vitest';
...
fs.writeFileSync(path.join(srcDir, 'services/TradingCalculationService.ts'), calcServiceCode);
fs.writeFileSync(path.join(srcDir, 'services/TradingCalculationService.test.ts'), testServiceCode);

console.log('TradingCalculationService and unit test file created successfully');
